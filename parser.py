...
# ============================================================
# Base de connaissance JSON unique
# ============================================================

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
KB_PATHS = [
    os.path.join(BASE_DIR, "security_knowledge.json"),
    os.path.join(BASE_DIR, "knowledge_base", "security_knowledge.json"),
]

def load_security_kb() -> List[Dict[str, Any]]:
...
SECURITY_KB = load_security_kb()

def find_kb_entry_exact(title: str) -> Optional[Dict[str, Any]]:
    if not title:
        return None
    ...
    for entry in SECURITY_KB:
        ...
        if isinstance(aliases, list):
            for alias in aliases:
                if normalize_text(alias) == norm_title:
                    return entry

    return None

def get_kb_description(title: str, default_description: str) -> str:
    entry = find_kb_entry_exact(title)
    if not entry:
        return default_description
    return str(entry.get("description") or default_description or "").strip()
# ============================================================
